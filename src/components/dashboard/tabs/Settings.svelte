<script lang="ts">
  ...
  /** Armed separately from `confirmWipe`: two destructive doors, never one. */
  ...
  /**
   * The rail, in the order someone actually goes looking.
   *
   * Danger zone is last and stays visually apart: it is the one section
   * where landing on it by accident matters.
   */
  const SECTIONS = [
    { key: "you", label: "You" },
  ...
  /** Which section is open. Resets to "You" on re-entry, which is fine -
      this component is rebuilt on every tab switch by design. */
  let section: (typeof SECTIONS)[number]["key"] = "you";
</script>

<div class="dsec-title">Settings</div>
<div class="dsec-sub">Everything you can control, in one place.</div>

<!-- Left rail, right panel. As one scroll it was a mile of unrelated
     switches - the wellness nudges sat between the workday length and the
     uninstall button, so finding anything meant reading everything. The rail
     names the eight places instead, and only one is on screen at a time. -->
<div class="setwrap">
  <nav class="setrail" aria-label="Settings sections">
    {#each SECTIONS as sec (sec.key)}
      <button
        class="setrail-b"
        class:on={section === sec.key}
        class:danger={sec.key === "danger"}
        aria-current={section === sec.key ? "true" : undefined}
        on:click={() => (section = sec.key)}
      >
        {sec.label}
      </button>
    {/each}
  ...
  <div class="setpane">
      ...
      <div class="settgrp">
        ...
        <div class="settrow">
          <div>
            ...
            <div class="sd">
              Used in greetings and a few bits of copy. Leave it empty and nothing anywhere says a
              name. Capped at {NAME_MAX} characters so it cannot push anything off screen.
            </div>
          </div>
          <input
            class="num-in"
        ...
        <div class="settrow">
          <div>
            ...
            <div class="sd">
              Printed at the top of an exported work record. Someone else reads that page, so a
              nickname is usually the wrong thing to put on it. Leave it empty and the line is
              omitted.
            </div>
          </div>
          <input
      ...
      <div class="settgrp">
        ...
        <div class="settrow">
          <div>
            ...
            <div class="sd">
              Remi's own accent comes from the app icon. Pick another if you'd rather.
            </div>
          </div>
        ...
        <div class="settrow">
          <div>
            ...
            <div class="sd">
              The mascot runs while your clock runs, sleeps through a break and cheers when the list
              is clear. Turn it off if movement in the corner of your eye is the last thing you
              need.
            </div>
          </div>
        ...
        <div class="settrow">
          <div>
            ...
            <div class="sd">
              Cosmetic only. Remi wears this everywhere except during the tour, which dresses itself
              for each page.
            </div>
          </div>
          <div class="cos-pick">
        ...
        <div class="settrow">
          <div>
            ...
            <div class="sd">
              ...
              because movement in the corner of your eye is the exact thing this app is meant to
              protect.
            </div>
          </div>
          <input
        ...
        <div class="settrow">
          <div>
            ...
            <div class="sd">
              Starting your day wakes Remi first - it rubs its eyes and settles at its desk before
              the dashboard opens. Always skippable with a click. Turn this off to start instantly.
            </div>
          </div>
          <input
            type="checkbox"
    ...
    {:else if section === "wellness"}
      <div class="settgrp">
        ...
        <div class="dsec-sub" style="margin:-2px 0 6px; font-size:11.5px;">
          Gentle, optional reminders to look after yourself. Off by default, one at a time, never
          during a break - and they never touch your task clock.
        </div>
      ...
      <div class="settgrp">
        ...
        <div class="settrow">
          <div>
            ...
            <div class="sd">
              One per line. Added fresh to every new day, skipping anything carried.
            </div>
          </div>
        </div>
      ...
      <div class="settgrp">
        ...
        <div class="settrow">
          <div>
            ...
            <div class="sd">
              Looks for a new release when Remi starts. It never installs anything on its own -
              updating is always a button you press, on the Data tab.
            </div>
          </div>
          <input
            type="checkbox"
      ...
      <div class="settgrp">
        ...
        <div class="settrow">
          <div>
            ...
            <div class="sd">
              A quick walk through adding tasks and steps, tagging, the clock, ending the day,
              reports and search. It runs on a sample day, so nothing you do in it touches your own.
              Runs once on a first launch; this is how you get it back.
            </div>
          </div>
          <div class="tour-cta">
            <TourSign size={210} />
    ...
    {:else if section === "danger"}
      <div class="settgrp">
        ...
        <!-- TWO actions, not one with a checkbox. They were a single
             "reset & uninstall" button, which made starting over cost you
             the app: the only way to get a clean slate was to quit and
             relaunch something you were in the middle of using. Resetting
             is for people who are STAYING; uninstalling is for people who
             are leaving. -->
        {#if !confirmWipe && !confirmReset}
          <div class="settrow">
            <div>
              <div class="st">Start Remi over</div>
              <div class="sd">
                Erase everything and go back to a first launch - the tour included. Remi stays open.
              </div>
            </div>
            <button class="set-btn danger" on:click={() => (confirmReset = true)}>
              Factory reset…
            </button>
          </div>
          <div class="settrow">
            <div>
              <div class="st">Uninstall Remi</div>
              <div class="sd">
                Remove Remi from this computer and quit, so you can drag the app to the Trash.
              </div>
            </div>
        ...
        {:else if confirmReset}
          <div class="bk-card" style="border-color:var(--danger);">
            <h4>Erase everything and start over?</h4>
            <p>
              Every day, streak, task, note, tag, export and setting goes. Remi stays open and comes
              back as it was the first time you opened it, tour and all. This cannot be undone.
            </p>
            <div class="bk-actions">
              <button class="set-btn" on:click={() => (confirmReset = false)}>Cancel</button>
              <button
                class="set-btn danger"
                on:click={async () => {
                  await factoryReset().catch(() => {});
                  confirmReset = false;
                }}
              >
          ...
          <div class="bk-card" style="border-color:var(--danger);">
            ...
            <label class="wipe-ask" class:armed={wipeHistory}>
              ...
              <span>
                ...
                <span class="wa-h">
                  {#if wipeHistory}
                    Everything goes: days, streaks, backlog, notes, exports, your name and every
                    setting. This cannot be undone.
                  {:else}
                    Your history, streaks, name and settings stay on this machine. Reinstall Remi
...
<style>
  /* Rail on the left, one section on the right. `align-items:start` keeps
     the rail from stretching to the height of a long pane. */
  .setwrap {
    display: grid;
  ...
  /* Narrow windows: the rail becomes a wrapping row above the pane. */
