...
describe("day lifecycle", () => {
  ...
  it("archives the day, carries the unfinished, and increments carries", () => {
    ...
    expect(s.carrySeed).toEqual([
      expect.objectContaining({ title: "Beta", carries: 1 }),
    ]);
  });
...
describe("streak revive", () => {
  it("spends the heart on the most recent genuinely-missed day", () => {
    ...
    s.history = [
      { day: 1, dateISO: "2026-08-10", completed: [{ title: "t", kind: "task", ms: 1 }], unfinished: [], totalMs: 1 },
      { day: 3, dateISO: "2026-08-12", completed: [{ title: "u", kind: "task", ms: 1 }], unfinished: [], totalMs: 1 },
    ];
    s.life = 1;
...
describe("import, export and restore", () => {
  it("applies a parsed import as tasks, steps and backlog", () => {
    applyImport({
      mains: [
        { title: "Imported", remind: null, subs: [{ title: "its step", remind: null }] },
      ],
