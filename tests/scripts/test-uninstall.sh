...
test_app_only_preserves_data() {
  ...
  HOME="$fake_home" bash "$UNINSTALL_SH" --app-path "$app_path" >/tmp/uninstall-out.$$ 2>&1
  local rc=$?
  ...
  rm -rf "$fake_home" /tmp/uninstall-out.$$
}
...
test_full_purge_removes_everything() {
  ...
  HOME="$fake_home" bash "$UNINSTALL_SH" --app-path "$app_path" --purge --yes >/tmp/uninstall-out.$$ 2>&1
  local rc=$?
...
test_purge_requires_yes() {
  ...
  HOME="$fake_home" bash "$UNINSTALL_SH" --app-path "$app_path" --purge >/tmp/uninstall-out.$$ 2>&1
  local rc=$?
...
test_repeated_uninstall_is_safe() {
  ...
  HOME="$fake_home" bash "$UNINSTALL_SH" --app-path "$app_path" >/tmp/uninstall-out1.$$ 2>&1
  local rc1=$?
  HOME="$fake_home" bash "$UNINSTALL_SH" --app-path "$app_path" >/tmp/uninstall-out2.$$ 2>&1
  local rc2=$?
  assert_eq "first uninstall exits 0" "0" "$rc1"
  assert_eq "second (repeated) uninstall also exits 0" "0" "$rc2"

  rm -rf "$fake_home" /tmp/uninstall-out1.$$ /tmp/uninstall-out2.$$
}
...
test_repeated_purge_is_safe() {
  ...
  HOME="$fake_home" bash "$UNINSTALL_SH" --app-path "$app_path" --purge --yes >/tmp/uninstall-out1.$$ 2>&1
  local rc1=$?
  HOME="$fake_home" bash "$UNINSTALL_SH" --app-path "$app_path" --purge --yes >/tmp/uninstall-out2.$$ 2>&1
  local rc2=$?
...
test_rejects_unsafe_paths() {
  ...
  HOME="$fake_home" bash "$UNINSTALL_SH" --app-path "$fake_home" >/tmp/uninstall-out.$$ 2>&1
  local rc=$?
